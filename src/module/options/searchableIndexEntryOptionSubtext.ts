...
import { FeatEntry, IndexEntry } from '../indexUtils';
...
export default class SearchableIndexEntryOption implements HeroOption {
  ...
  constructor(
    ...
    readonly key: string,
    private options: Array<FeatEntry>,
  ...
  showSuggestions(searchArray: Array<FeatEntry>) {
    let listData;
        ...
        .map(
          (result) =>
            `<li>
              ...
              <div data-key=\"${result._id ?? result.name}\">
                <i style = "font-size: 15px; font-weight: italics; color: black;">${result.data.requirements ?? ''}</i>
              </div>
            </li>`,
        )
        .join('');
